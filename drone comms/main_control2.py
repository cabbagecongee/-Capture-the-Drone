...
def readserial(comport):
    ser = serial.Serial(port=comport, baudrate=9600, timeout=0.1)         # 1/timeout is the frequency at which the port is read
    while True:
...
def getControllerInput(x, y, button1, z, rotation, button2):
    ...
    if x < 515: lr = -speed #left right
    elif x > 515: lr = speed

    if y > 515: fb = moveSpeed #forward backward
    elif y < 515: fb = -moveSpeed

    if z > 515: ud = liftSpeed #up down
    elif z < 515: ud = -liftSpeed 

    if rotation < 494: yv = rotationSpeed #left right rotation
    elif rotation > 494: yv = -rotationSpeed 

    if button1 == 0: drone.land(); 
    elif button2 == 0: drone.takeoff() 
...
while True:
    ...
    if __name__ == '__main__':
        controller_data = readserial('COM4')
        x, y, button1, z, rotation, button2 = controller_data
    else:
        print("No data from COM4 detected.")
    
    # keyValues = getKeyboardInput() 
    keyValues = getControllerInput() 
